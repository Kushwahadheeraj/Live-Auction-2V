  ...
  const handleDeleteAuction = async (id) => {
    if (window.confirm('Are you sure to delete?')) {
      try {
        ...
        dispatch({
          type: 'FETCH_SUCCESS',
          payload: products.filter((auctionitem) => auctionitem._id !== id),
        });
      ...
      <main className="container mx-auto py-8">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {products.reverse().map((auctionitem) => (
              <div className="border-b border-gray-200 py-2" key={auctionitem._id}>
                {(auctionitem.bids.length > 0 &&
                  auctionitem.bids[auctionitem.bids.length - 1]?.bidder) ===
                userInfo?.name ? (
                  <AuctionItem
                    key={auctionitem._id}
                    id={auctionitem._id}
                    ...
                    handleDeleteAuction={() => handleDeleteAuction(auctionitem._id)}
                  />
                ) : (
                  <AuctionItem
                    key={auctionitem._id}
